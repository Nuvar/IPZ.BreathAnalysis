...
namespace WPFChart3D
{
    ...
    public partial class Window1 : Window
    {
        ...
        {
            using (WaveFileReader reader = new WaveFileReader(@"C:\Users\Vlad\Downloads\Music\Casio-MT-45-16-Beat.wav"))
            {
                byte[] buffer = new byte[reader.Length];
                int read = reader.Read(buffer, 0, buffer.Length);
                var signal = new short[read/2];
                Buffer.BlockCopy(buffer, 0, signal, 0, read);
                int duration = (int) reader.TotalTime.TotalMilliseconds;
                
                var test = new Wavelet();
                var res = test.wavelet_transformation(duration, signal, 44,
                test.Morlet_wavelet, 100);
                double[,] arr = new double[(int)res.Max(f => f.X) + 1, (int)res.Max(f => f.Y) + 1];
                m_3dChart = new UniformSurfaceChart3D();
                ((UniformSurfaceChart3D)m_3dChart).SetGrid(res.Count, res.Count, 0, (int)res.Max(x => x.X), 0, (int)res.Max(x => x.Y));
                ...
                {
                    m_3dChart[i].x = (float) res[i].X;
                    m_3dChart[i].y = (float) res[i].Y;
                    m_3dChart[i].z = (float) res[i].Z;
                }
